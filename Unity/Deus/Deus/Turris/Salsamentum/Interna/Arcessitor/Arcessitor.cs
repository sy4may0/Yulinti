...
namespace Yulinti.Unity.Deus {
    internal sealed class Arcessitor {
        ...
        private static readonly string _DirPathInvalid = Path.Combine(Application.persistentDataPath, ConstansDeus.DirPathSalsamentumInvalid);
        ...
        public Arcessitor(IScriba scriba, ThesaurusSalsamenti thesaurusSalsamenti) {
            ...
            _bufServata = new (string json, string path, string tmp)[ConstansDeus.MaximusNumerusDatumServatum];
            _bufLiberata = new (string path, string tmp)[ConstansDeus.MaximusNumerusDatumServatum];
            _adSalva = AdSalva;
            _adDele = AdDele;
        ...
        private void AdSalva(string json, string path, string tmp) {
            if (_indexusSalva >= ConstansDeus.MaximusNumerusDatumServatum) {
        ...
        private void AdDele(string path, string tmp) {
            if (_indexusDele >= ConstansDeus.MaximusNumerusDatumServatum) {
